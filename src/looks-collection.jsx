...
function LookCard({ look, index, isOutfit, items = [], onOpen, onDelete }) {
  const pieces = useMemo(
    () => isOutfit ? (look.itemIds || []).map((id) => items.find((item) => item.id === id)).filter(Boolean) : [],
    [isOutfit, look.itemIds, items],
  );
  ...
  return (
    <article className="look-card">
      <button className="look-card__image" type="button" onClick={() => onOpen(look)} aria-label={`Otvori ${look.name} preko cijelog ekrana`}>
        ...
        {!!pieces.length && (
          <span className="look-card__hover-pieces" aria-hidden="true">
            {pieces.map((piece) => <img key={piece.id} src={piece.thumbnail || piece.image} alt="" loading="lazy" decoding="async" />)}
          </span>
        )}
      </button>
          ...
          <section className="looks-section" aria-labelledby="looks-outfits-title">
            ...
            {outfitLooks.length ? (
              <div className="looks-grid">
                {outfitLooks.map((look, index) => <LookCard key={look.id} look={look} index={index} isOutfit items={items} onOpen={setSelected} onDelete={removeLook} />)}
              </div>
            ) : (
